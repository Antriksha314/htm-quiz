let points = 0;
const ausCapital = "CANBERRA";
const canCapital = "OTTAWA";
function australia(userAnswer) {
  // what is the capital of australia?
  // 100 pts

  const isCorrect = userAnswer.toUpperCase() === ausCapital;
  if (isCorrect) {
    points += 100;
  }
  return isCorrect;
...
function canada(userAnswer) {
  ...
  const isCorrect = userAnswer.toUpperCase() === canCapital;
  if (isCorrect) {
    points += 100;
  }
  return isCorrect;
...
function firstThreeLetters(userAnswer, word) {
  ...
  const firstThreeLetter = word.substr(0, 3);
  const isCorrect = userAnswer.toUpperCase() === firstThreeLetter.toUpperCase();
  if (isCorrect) {
    points += 200;
  }
  return isCorrect;
...
function age(userAnswer, currentYear, birthYear) {
  ...
  const isCorrect = userAnswer == age;
  if (isCorrect) {
    points += 300;
  }
  return isCorrect;
...
function larger(userAnswer, num1, num2) {
  ...
  const isCorrect = userAnswer == large;
  if (isCorrect) {
    points += 300;
  }
  return isCorrect;
